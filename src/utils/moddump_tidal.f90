...
subroutine modify_dump(npart,npartoftype,massoftype,xyzh,vxyzu)
 ...
 real                    :: cos_f0, sin_f0  ! 添加真近点角的余弦和正弦值
!
...
if (ecc < 1.) then
    ...
    semia    = rp/(1.-ecc)
    period   = 2.*pi*sqrt(semia**3/m0)  ! 修正：使用m0而不是mass1
    
    ! 计算正确的真近点角
    cos_f0 = (semia*(1.-ecc**2)/r0 - 1.)/ecc
    sin_f0 = -sqrt(1. - cos_f0**2)

    ! y=-x x=y
    x0 =  r0*sin_f0
    y0 = -r0*cos_f0
    z0 = 0.
    vx0 = sqrt(m0/(semia*(1.-ecc**2)))*(ecc+cos_f0)
    vy0 = sqrt(m0/(semia*(1.-ecc**2)))*sin_f0
    vz0 = 0.
    xyzstar = (/x0,y0,z0/)
 ...
 if (nptmass==2) then
   xyzmh_ptmass(1,1) = xyzmh_ptmass(1,1) + x0
   xyzmh_ptmass(2,1) = xyzmh_ptmass(2,1) + y0
   xyzmh_ptmass(3,1) = xyzmh_ptmass(3,1) + z0
   vxyz_ptmass(1,1) = vxyz_ptmass(1,1) + vx0
   vxyz_ptmass(2,1) = vxyz_ptmass(2,1) + vy0
   vxyz_ptmass(3,1) = vxyz_ptmass(3,1) + vz0
 endif
